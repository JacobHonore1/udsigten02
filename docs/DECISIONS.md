...
## 46. Domæneflytning til udsigten.dk — deploy target, subscribe.php og GDPR-mikrotekst

Sitet flytter fra `zaxis.dk/udsigten02` til sit eget domæne, `udsigten.dk`, hostet i Simplys root (`/public_html/`) i stedet for en undermappe.

- **`.github/workflows/deploy.yml`:** `server` ændret til `ftp.simply.com`, `username` til `udsigten.dk`, `server-dir` til `/public_html/` (fra `/public_html/udsigten02/`). `password` peger nu på et nyt GitHub Secret, `FTP_PASSWORD_UDSIGTEN` (oprettet af brugeren), i stedet for `FTP_PASSWORD`. **`FTP_PASSWORD`-secret'et er bevidst IKKE slettet** — det skal genbruges senere til en redirect på det gamle `zaxis.dk`-hotel.
- **`subscribe.php`:** Modtagerlisten reduceret fra tre (`info@udsigten.dk, epl@seguro.dk, jh@zaxis.dk`) til kun `info@udsigten.dk`. `From`-headeren ændret fra `jh@zaxis.dk` til `noreply@udsigten.dk` (nu det korrekte afsenderdomæne efter flytningen — punkt 45's anti-spoofing-logik om at bruge en reel postkasse på det *aktive* domæne gælder fortsat, men domænet er nu `udsigten.dk`). Ny CSV-logning tilføjet: hver tilmelding appendes til `/data/tilmeldinger.csv` (én mappeniveau over `public_html`, uden for web root) via `fputcsv`, med header-række (`E-mail`, `Tidspunkt`) skrevet første gang filen oprettes. Mail-afsendelse fejler ikke selvom CSV-skrivning fejler (logges blot med `error_log`) — de to er uafhængige.
- **GDPR-mikrotekst:** Ny `<p class="hero-signup-disclaimer">Din e-mail bruges kun til at kontakte dig om Udsigten Haderslev.</p>` indsat i `index.html` efter `.hero-signup`s lukkende `</div>`, som sidste element i `.hero`-sektionen før `</section>`. Da elementet er i normal dokumentflow (ikke absolut positioneret som `.hero-content`/`.hero-signup`), lægger den sig visuelt under hero-billedet uden at påvirke den eksisterende absolutte positionering. CSS: `font-size: 11px`, `color: rgba(26, 20, 16, 0.45)` (samme faded-værdi som `.hero-signup-input::placeholder`), højrestillet med `padding-right` der matcher `.hero-signup`s `right`-offset på desktop; centreret uden padding i mobil-medieforespørgslen, hvor `.hero-signup` selv bliver centreret (jf. punkt 36).

**Status: implementeret. Ikke verificeret live endnu** — kræver at `FTP_PASSWORD_UDSIGTEN`-secret'et virker og at `udsigten.dk` reelt peger på samme Simply-konto/mappe.

## 47. Event-banner og separat tilmeldingsliste: Åbent Hus, Arkitekturens Dag (5. oktober 2026) — MIDLERTIDIG

Kunden bad om et event-banner på forsiden for "Arkitekturens Dag" (mandag d. 5.10.26, kl. 16.30-18.00, åbent hus i Sengetårnet) samt en tilmeldingsformular til arrangementet. Krav fra kunden: den nye tilmeldingsliste skal være **fuldstændig separat** fra den generelle boliginteresse-liste (`subscribe.php`/`tilmeldinger.csv`) — intet i den eksisterende flow må ændres.

- **`.event-banner`:** ny fast (`position: fixed; top: 0`) bjælke over `.site-header`, baggrund `var(--accent)` (`#8B5E3C`), hele bjælken er et `<a href="#event">`-link der scroller ned til event-sektionen. Ny CSS-variabel `--banner-height` (`44px` desktop, `68px` under 768px hvor teksten ombrydes til to linjer) bruges til at skubbe `.site-header` (`top: var(--banner-height)`) og `.hero`s margin-top tilsvarende ned, så intet overlapper.
- **Event-sektion (`#event`):** placeret lige efter `.hero`, før Vision-boksen. Genbruger eksisterende mønstre for at "blende ind": `.section-block` med `border-left: 8px solid var(--accent)` (ingen kategorifarve, da arrangementet ikke hører til Boliger/Hotel/Spa/Svømmehal), `.feature-list` til rundvisnings-tidspunkter, og `.kontakt-form`-klassen genbruges direkte på det nye `.event-signup`-formular for identisk input/knap-styling uden ny CSS at vedligeholde.
- **Formular:** indsamler navn, e-mail, valgt rundvisnings-tidspunkt (`<select>`, tvunget valg mellem "1. hold — kl. 16.30" og "2. hold — kl. 17.15", ingen forudvalgt værdi) og antal deltagere (talfelt, tomt som udgangspunkt så placeholder "Antal deltagere" er synlig — ikke forudfyldt med `1`). `.kontakt-form`-klassen udvidet med `select`-styling (samme padding/border/font som `input`/`textarea`; `select:invalid` farves som `--muted` så det uudfyldte "Vælg tidspunkt" ser ud som en placeholder). Egen GDPR-mikrotekst i samme stil som punkt 46: "Dine oplysninger bruges kun til at kontakte dig om dette arrangement."
- **`event-subscribe.php`:** ny, selvstændig fil — kopi af `subscribe.php`-mønsteret, ikke en ændring af den. Validerer `timeslot` mod en fast liste (`16:30`/`17:15`) server-side, ikke kun i UI'et. Skriver til sin egen CSV, `/data/arkitekturens-dag.csv` (samme mappe som `tilmeldinger.csv`, men egen fil, kolonner: Navn, E-mail, Rundvisning, Antal deltagere, Tilmeldt). Sender mail til `nyt@udsigten.dk` med `Cc: epl@seguro.dk`, `From: noreply@udsigten.dk` (samme afsenderdomæne-logik som punkt 45/46).
